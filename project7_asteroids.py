...
def draw(canvas):
    ...
    for missile in missile_group:
        missile.update()
        if missile.get_age() == M_AGE_MAX:
            rem_miss.add(missile)
    for r in rem_miss:
        missile_group.remove(r)
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started == True:
        ...
        rock_vel = [random.random() - .3, random.random() - .3]
        rock_avel = random.random() * .2 - .1
        rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        if dist(rock.get_pos(),my_ship.get_pos()) > 100:
            rock_group.add(rock)
...
def test_collision():
    ...
    for rock in rock_group:
        cent = rock.get_pos()
        if dist(cent,my_ship.get_pos()) <= 90:
            Kill_Ship()
            rock_hit.add(rock)
        for m in missile_group:
            if dist(cent,m.get_pos()) <= 45:
                missile_hit.add(m)
                rock_hit.add(rock)
                score +=1
    
    rock_group.difference_update(rock_hit)
    missile_group.difference_update(missile_hit)
